			...
			if(!HIDTxHandleBusy(USBInHandle))
			{
				switch(ReceivedDataBuffer[1])
		        {
			        case TRAMA_STATUS:	// 2
						...
						if (Estoy_Test == 0) Aplicador 	= ReceivedDataBuffer[8];
						Senyal_Stim = ReceivedDataBuffer[9];
						
						if ((Estado_RF == O_N)||(Estado_Galva == O_N))
						{
							if (No_Apli == 1)	// TODO SIVA hacer de alguna manera fuera, que intab se entere que no apli
							{
								Estado_Maquina = 0;
								Cont_Apli = 0;
							}
...
#endif
						if ((Reducir == 1)&&(Valor_RF >= 2)) Valor_RF = 1;//Valor_RF / 2;
