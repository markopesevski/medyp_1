...
unsigned int Lectura_Probe (unsigned char Canal_Prob)	// OK
{
	...
	{
		case PROBE1:
			AD1CHS = (ADC_CH0_POS_SAMPLEA_AN5 | ADC_CH0_NEG_SAMPLEA_NVREF);
		break;
		case PROBE2:
			AD1CHS = (ADC_CH0_POS_SAMPLEA_AN4 | ADC_CH0_NEG_SAMPLEA_NVREF);
		break;
		case PROBE3:
			AD1CHS = (ADC_CH0_POS_SAMPLEA_AN9 | ADC_CH0_NEG_SAMPLEA_NVREF);
		break;
		default:
		break;
	}
...
void Mirar_Aplicador (void)
{
	...
	else if ((Valor_Probe1 < 483)&&(Valor_Probe1 > 425))
	{
		Salida_Uno = FACIAL;//448
	}
	else if ((Valor_Probe1 < 565)&&(Valor_Probe1 > 495))
	{
		Salida_Uno = CORPORAL;//530
	}
	else if (Valor_Probe1 > 750)
	{
		Salida_Uno = 0;//986 nc
	}
	...
	else if ((Valor_Probe2 < 483)&&(Valor_Probe2 > 425))
	{
		Salida_Dos = FACIAL;
	}
	else if ((Valor_Probe2 < 565)&&(Valor_Probe2 > 495))
	{
		Salida_Dos = CORPORAL;
	}
	else if (Valor_Probe2 > 750)
	{
		Salida_Dos = 0;
	}
	...
	else if ((Valor_Probe3 < 483)&&(Valor_Probe3 > 425))
	{
		Salida_Tres = FACIAL;
	}
	else if ((Valor_Probe3 < 565)&&(Valor_Probe3 > 495))
	{
		Salida_Tres = CORPORAL;
	}
	else if (Valor_Probe3 > 750)
	{
		Salida_Tres = 0;
	}
...
void Activar_Salida (void)
{
	...
	if ((Salida_Uno == 0)&&(Salida_Dos == 0)&&(Salida_Tres == 0))
	{
		No_Apli = 1;	// TODO el aplicador que toca mirar!!!!
	}
	else No_Apli = 0;
}
...
void Control_Temperatura (void)
{
	...
	if (Primera_Temp == 0)
	{
		...
		for (Cont_Temp=0;Cont_Temp<10;Cont_Temp++)
		{
			Vector_Temp[Cont_Temp] = Temperatura;
		}
		Cont_Temp = 0;
	}
	...
	{
		if (Temperatura < (Temperatura_Alarma - 10))
		{
			Alarma_Temp = 0;
		}
	}
	...
	else
	{
		if (Segundos >= SEGS_HS)// Apago pasado un minuto seguido sin llegar a la temperatura.
		{
			if (Estado_RF == OFF)
			{
				HS_FAN = 0;	// Apago ventilador radiador y no está la RF en marcha.
			}
			Segundos = SEGS_HS;
		}
	}
