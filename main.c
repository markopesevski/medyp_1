...
unsigned char Mi_Estado_Maquina = 2;	// Empiezo en standby
	...
	while(1)
	{
		...
		if (Estado_Galva == O_N)	// OK
		{
			if (Sem.Lect_Gal == 1)
			{
				...
				if (((tickCambio > 10)&&(tickCambio < 190))||((tickCambio > 212)&&(tickCambio < 390)))
				{
					...
					if (Media_Galva <= 50) Retorno_Galva = 0;
					else if ((Media_Galva > 50)&&(Media_Galva <= 135)) Retorno_Galva = 1;	//90
						else if ((Media_Galva > 135)&&(Media_Galva <= 225)) Retorno_Galva = 2;	//180
							else if ((Media_Galva > 225)&&(Media_Galva <= 315)) Retorno_Galva = 3;	//270
								else if ((Media_Galva > 315)&&(Media_Galva <= 405)) Retorno_Galva = 4;	//360
									else if ((Media_Galva > 405)&&(Media_Galva <= 495)) Retorno_Galva = 5;	//450
										else if ((Media_Galva > 495)&&(Media_Galva <= 585)) Retorno_Galva = 6;	//540
											else if ((Media_Galva > 585)&&(Media_Galva <= 675)) Retorno_Galva = 7;	//630
												else if ((Media_Galva > 675)&&(Media_Galva <= 765)) Retorno_Galva = 8;	//720
													else if ((Media_Galva > 765)&&(Media_Galva <= 855)) Retorno_Galva = 9;	//810
														else Retorno_Galva = 10;	//900
														
					if (Cont_Galva < 9) Cont_Galva++;
					else Cont_Galva = 0;
				}							
			}		
		}
		...
		if (Sem.Lect_Man == 1)	// Cada 200 ms miro aplicadores.
		{
			...
			if (Blink_PB == 2)
			{
				...
				{
					if (Standby == 1) Led.Bit.LedPB = 1;
					else Led.Bit.LedPB = 0;
				}	
			Sem.Lect_Man = 0;
			Mirar_Aplicador();
			Activar_Salida();
		...
		{		
			if (Sem.Flag_Barrido == 1)
			{
				...
				if (Subiendo == 0)
				{
					...
					if (Frecuencia_Barrido >= 300) Subiendo = 1;
				}
				else
				{
					Frecuencia_Barrido = Frecuencia_Barrido - 5;
					if (Frecuencia_Barrido <= 100) Subiendo = 0;
				}
			...
			switch (Senyal_Stim)
			{
				case 4:
					if (Aux_Contador_Stim != Contador_Stim)
					{
						...
						Modular++;
						if (Contador_Tim >= 21500) Cont_Mod = 0;
		...
		if (Sem.Temper == 1)	// Miro temperatura cada 1 segundo.	// TODO se ha movido a cada 200ms se podria quitar?
		{
			if (Estoy_Test != 0)
			{
				...
				if (Cont_Test >= 10)
				{
					 ...
					 Cont_Test = 0;
					 Aplicador = 0;
					 Ant_Estado_Maquina = 3; // Para parar los test.
				}	 
			}
...
//			Control_Temperatura();			
	...
	//			tick_Tecla = 0;
			...
			{	
				...
				{
					if ((Mi_Estado_Maquina == 0)&&(Estado_Maquina != 1)&&(Standby == 0))	// Voy a standby si no estoy en tratamiento.
					{
						Led.Bit.LedPB = 1;
						Mi_Estado_Maquina = 2;
						Blink_PB = 2;
					}
					if ((Mi_Estado_Maquina == 2)&&(Standby == 1))	// Despierto de standby.
					{
						Led.Bit.LedPB = 0;
						Mi_Estado_Maquina = 0;
						Blink_PB = 2;
				...
				{	
					...
					{
						if (Mi_Estado_Maquina == 2) Standby = 1;
						if (Mi_Estado_Maquina == 0) Standby = 0;
					}
					if (tick_Tecla >= 40) Blink_PB = 3;
				}
			}
		}
...
void Init_Regs (void)
{
	...
	if (Num_Serie > 0xFFFFFF) Num_Serie = 0;
	
	Volumen = *(int *)(NVM_PROGRAM_PAGE + 8);
	if (Volumen > 10) Volumen = VOL_FAB;
	Control_Volumen(Volumen);
	
	Dac_Gal = *(int *)(NVM_PROGRAM_PAGE + 12);
	if (Dac_Gal >= 255) Dac_Gal = DAC_GAL_FAB;
	
	Dac_Stim = *(int *)(NVM_PROGRAM_PAGE + 16);
	if (Dac_Stim >= 255) Dac_Stim = DAC_STIM_FAB;
	
	Dac_RF = *(int *)(NVM_PROGRAM_PAGE + 20);
	if (Dac_RF >= 255) Dac_RF = DAC_RF_FAB;
	
	Dac_Bias = *(int *)(NVM_PROGRAM_PAGE + 24);
	if (Dac_Bias >= 255) Dac_Bias = DAC_BIAS_FAB;
	
	Temperatura_Alarma = *(int *)(NVM_PROGRAM_PAGE + 28);
	if (Temperatura_Alarma >= 255) Temperatura_Alarma = TEMPERATURA_ALARMA;
